...
from vqvae.nearest_embed import NearestEmbed, NearestEmbedEMA
...
class VQ_VAE(nn.Module):
    ...
    def __init__(self, in_size=64*64, hidden=200, k=10, vq_coef=0.2, comit_coef=0.4, **kwargs):
        super(VQ_VAE, self).__init__()

        self.emb_size = k
        self.in_size = in_size
        self.fc1 = nn.Linear(in_size, 256)
        self.fc1_5 = nn.Linear(256, 256)
        self.fc2 = nn.Linear(256, hidden)
        self.fc3 = nn.Linear(hidden, 256)
        self.fc3_5 = nn.Linear(256, 256)
        self.fc4 = nn.Linear(256, in_size)
    ...
    def encode(self, x):
        ...
        h2 = self.lrelu(self.fc1_5(h1))
        h3 = self.fc2(h2)
        return h3.view(-1, self.emb_size, int(self.hidden / self.emb_size))

    def decode(self, z):
        h3 = self.lrelu(self.fc3(z))
        h4 = self.lrelu(self.fc3_5(h3))
        return self.lrelu(self.fc4(h4))

    def forward(self, x):
        z_e = self.encode(x.view(-1, self.in_size))
    ...
    def loss_function(self, x, recon_x, z_e, emb):
        self.ce_loss = F.mse_loss(recon_x, x.view(-1, self.in_size))
        self.vq_loss = F.mse_loss(emb, z_e.flatten(1).detach())
        self.commit_loss = F.mse_loss(z_e.flatten(1), emb.detach())

        return self.ce_loss + self.vq_coef*self.vq_loss + self.comit_coef*self.commit_loss

    def latest_losses(self):
        return {'mse': self.ce_loss, 'vq': self.vq_loss, 'commitment': self.commit_loss}
...
class CVAE(AbstractAutoEncoder):
    def __init__(self, d, kl_coef=0.1, **kwargs):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(3, d // 2, kernel_size=(5,10), #kernel_size=4,
                      stride=2, padding=1, bias=False),
            nn.BatchNorm2d(d // 2),
            nn.ReLU(inplace=True),
            nn.Conv2d(d // 2, d, kernel_size=(5,10), # kernel_size=4,
                      stride=2, padding=1, bias=False),
            nn.BatchNorm2d(d),
            nn.ReLU(inplace=True),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(d, d // 2, kernel_size=(5,10), #kernel_size=4,
                               stride=2, padding=1, bias=False),
            nn.BatchNorm2d(d//2),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(d // 2, 3, kernel_size=(5,10), # kernel_size=4,
                               stride=2, padding=1, bias=False),
        )
        self.f = 8
        self.d = d
...
class VQ_CVAE(nn.Module):
    def __init__(self, d, k=10, bn=True, vq_coef=1, commit_coef=0.5, num_channels_in=3, num_channels_out=3, **kwargs):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.BatchNorm2d(d),
            ResBlock(d, d),
            nn.ConvTranspose2d(d, d, kernel_size=48, stride=1, padding=1),
            nn.BatchNorm2d(d),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(
                d, num_channels_out, kernel_size=48, stride=1, padding=1),
        )
        self.d = d
