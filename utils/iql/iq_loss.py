...
def iq_loss(agent, current_Q, current_v, next_v, gamma, batch):
    # args = agent.args
    # gamma = agent.gamma
    ...
    phi_val_loss = phi_loss + value_loss
    ...
    reward = current_Q - y
    chi2_loss = 1/(4 * 0.5) * (reward**2)[is_expert].mean()
    phi_chi_val_loss = phi_val_loss + chi2_loss
    ...
    # print(loss)
    return phi_chi_val_loss, loss_dict